"""
Fol-Pong: Un juego de Pong implementado con lógica de primer orden.

Este módulo implementa un juego tipo Pong utilizando la biblioteca pyDatalog
para el razonamiento lógico que determina las acciones y el comportamiento del juego.
"""
...
# Hechos iniciales - usamos el operador + para aserciones
...
GolpeaPelota(X, Y) <= (PosicionPelota(X, Y) &
                       PosicionRaqueta(Y) &
                       (X == POSICION_X_RAQUETA))


# Consultas
print("\n--- Estado Actual del Juego ---")
# Se anexan comprobaciones
pelota_pos_result = pyDatalog.ask('PosicionPelota(X, Y)')
print("Posición de la pelota:",
      pelota_pos_result.answers if pelota_pos_result else "No disponible")

raqueta_pos_result = pyDatalog.ask('PosicionRaqueta(Y)')
print("Posición de la raqueta:",
      raqueta_pos_result.answers if raqueta_pos_result else "No disponible")

direccion_result = pyDatalog.ask('DireccionPelota(DX, DY)')
print("Dirección de la pelota:",
      direccion_result.answers if direccion_result else "No disponible")


# Consulta si la pelota está en la zona de la raqueta/paleta
...
if pelota_en_zona:
    ...
    print("¿Pelota en zona de raqueta?: No - La pelota no está en "
          "la zona de la raqueta")


# Consultas sobre el movimiento de la raqueta/paleta
